...
WITH Unpivoted AS (
    SELECT 
        ...
        val
    FROM Contacts
    UNPIVOT (val FOR col IN (' + @columns + ')) AS unpvt
),
JsonData AS (
    SELECT 
        identifier_name,
        ...
        CAST(
            ...
            ''"properties":['' +
            STRING_AGG(
                CAST(''{"property":"'' + col + ''","value":"'' + val + ''"}'' AS NVARCHAR(MAX)),
                '',''
            ) +
...
SELECT CAST(
    ''['' + 
    STRING_AGG(CAST(json_output AS NVARCHAR(MAX)), '','') + 
    '']'' 
    AS NVARCHAR(MAX)) AS final_json
FROM JsonData;
';
